...
const uint32_t * __unprivileged_functions_start__		= ( uint32_t * ) &( Image$$ER_IROM_NS_PRIVILEGED_EXECUTE_NEVER$$Base );
const uint32_t * __unprivileged_functions_end__			= ( uint32_t * ) ( ( uint32_t ) &( Image$$ER_IROM_NS_PRIVILEGED_EXECUTE_NEVER_ALIGN$$Limit ) - 0x1 ); /* Last address in privileged execute never Flash region. */
...
static void prvPacTask( void * arg )
{
    ...
    #if ( ( ARM_V_8_1_M_PACBTI_CONFIG == ARM_V_8_1_M_PACBTI_CONFIG_STANDARD ) || \
