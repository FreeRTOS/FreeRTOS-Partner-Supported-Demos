...
Two FreeRTOS tasks, each pinned to a different core, exchange a shared flag and print **Ping / Pong** messages to the console to prove that:
	...
	• Inter-core communication through a cache-coherent shared RAM region works.
...
| Item | Description |
|------|-------------|
| **Shared flag** | `ulSharedFlag` (64-bit) is placed in a `.shared_ram` section so it is cache-coherent across cores. |
...
| **Scheduler bring-up** | Core 0 creates the tasks and calls `vTaskStartScheduler()`. Secondary cores spin in `wfe` until `uxPortSchedularRunning` becomes `pdTRUE`, then invoke `xPortStartScheduler()`. |
