...
#if defined( __GNUC__ ) && !defined( __clang__ )
    static struct _reent core_reent_array[configNUMBER_OF_CORES];
#endif /* if defined( __GNUC__ ) && !defined( __clang__ ) */
...
int main()
{
    ...
    {
        while( ucPortSchedulerRunning != pdTRUE )
        {
            __asm__ volatile ( "wfe" );
        }
        #if defined( __GNUC__ ) && !defined( __clang__ )
            uint32_t ulCoreID = xPortGetCoreID();      /* 1, 2, 3… */
            if ( ulCoreID < configNUMBER_OF_CORES )
            {
                struct _reent *pxReent = &core_reent_array[ ulCoreID ];
                _REENT_INIT_PTR( pxReent );                /* zero & set default locale etc. */
                _impure_ptr = pxReent;                     /* make Newlib use this copy      */
            }
            else
            {
                printf( "Invalid core ID: %u\r\n", ulCoreID );
                return EXIT_FAILURE;
            }
        #endif /* if defined( __GNUC__ ) && !defined( __clang__ ) */
...
void vApplicationIRQHandler( uint32_t ulICCIAR )
{
    ...
    else if( ulInterruptID == PPI0_IRQ )
    {
